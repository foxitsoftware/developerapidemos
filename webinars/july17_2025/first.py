"""
First demo - add an upload method
add a word to pdf method
add a check tag method
...
doc = uploadDoc("dell-monitor.pdf", CLIENT_ID, CLIENT_SECRET)
print(f"Uploaded doc to Foxit, id is {doc['documentId']}")

job = optimizePDF(doc['documentId'], CLIENT_ID, CLIENT_SECRET)

result = checkTask(job["taskId"], CLIENT_ID, CLIENT_SECRET)
print("Done linearizing PDF.")

downloadResult(result['resultDocumentId'], 'dell-lin.pdf', CLIENT_ID, CLIENT_SECRET)
print("Done downloading linearized PDF.")