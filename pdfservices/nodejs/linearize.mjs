...
let task = await linearizePDF(doc.documentId, CLIENT_ID, CLIENT_SECRET, "HIGH");
console.log(`Created task, id is ${task['taskId']}`);

let result = await checkTask(task.taskId, CLIENT_ID, CLIENT_SECRET);
console.log(`Final result: ${JSON.stringify(result)}`);

await downloadResult(result.resultDocumentId, "../../output/node_input_linearized.pdf", CLIENT_ID, CLIENT_SECRET);
console.log("Done and saved to: ../../output/node_input_linearized.pdf");