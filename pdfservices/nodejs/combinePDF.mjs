...
async function combinePDF(docs, id, secret) {
	...
	let resp = await fetch(`${HOST}/pdf-services/api/documents/enhance/pdf-combine`, {
		method:"POST",
		headers, 
...
let doc1 = await uploadDoc("../../inputfiles/input.pdf", CLIENT_ID, CLIENT_SECRET)
console.log(`Uploaded doc to Foxit, id is ${doc1['documentId']}`);

let doc2 = await uploadDoc("../../inputfiles/second.pdf", CLIENT_ID, CLIENT_SECRET)
console.log(`Uploaded second doc to Foxit, id is ${doc2['documentId']}`);

let task = await combinePDF([doc1,doc2], CLIENT_ID, CLIENT_SECRET);
console.log(`Created task, id is ${task['taskId']}`);

let result = await checkTask(task.taskId, CLIENT_ID, CLIENT_SECRET);
console.log(`Final result: ${JSON.stringify(result)}`);

await downloadResult(result.resultDocumentId, "../../output/node_input_combined.pdf", CLIENT_ID, CLIENT_SECRET);
console.log("Done and saved to: ../../output/node_input.pdf");