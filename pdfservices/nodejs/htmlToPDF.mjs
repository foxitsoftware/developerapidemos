...
async function htmlToPDF(doc, id, secret) {
	...
	let resp = await fetch(`${HOST}/pdf-services/api/documents/create/pdf-from-html`, {
		method:"POST",
		headers, 
...
let doc = await uploadDoc('../../inputfiles/input.html', CLIENT_ID, CLIENT_SECRET);
console.log(`Uploaded doc to Foxit, id is ${doc['documentId']}`);

let task = await htmlToPDF(doc.documentId, CLIENT_ID, CLIENT_SECRET);
console.log(`Created task, id is ${task['taskId']}`);

let result = await checkTask(task.taskId, CLIENT_ID, CLIENT_SECRET);
console.log(`Final result: ${JSON.stringify(result)}`);

await downloadResult(result.resultDocumentId, "../../output/node_fromhtml.pdf", CLIENT_ID, CLIENT_SECRET);
console.log("Done and saved to: ../../output/node_fromhtml.pdf");