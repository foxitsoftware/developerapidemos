...
async function convertToPDF(doc, id, secret) {
	...
	let resp = await fetch(`${HOST}/pdf-services/api/documents/create/pdf-from-word`, {
		method:"POST",
		headers, 
...
let doc = await uploadDoc('../../inputfiles/input.docx', CLIENT_ID, CLIENT_SECRET);
console.log(`Uploaded doc to Foxit, id is ${doc['documentId']}`);

let task = await convertToPDF(doc.documentId, CLIENT_ID, CLIENT_SECRET);
console.log(`Created task, id is ${task['taskId']}`);

let result = await checkTask(task.taskId, CLIENT_ID, CLIENT_SECRET);
console.log(`Final result: ${JSON.stringify(result)}`);

await downloadResult(result.resultDocumentId, "../../output/node_input.pdf", CLIENT_ID, CLIENT_SECRET);
