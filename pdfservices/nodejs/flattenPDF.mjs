...
async function flattenPDF(doc, id, secret) {
	...
	let resp = await fetch(`${HOST}/pdf-services/api/documents/modify/pdf-flatten`, {
		method:"POST",
		headers, 
...
let task = await flattenPDF(doc.documentId, CLIENT_ID, CLIENT_SECRET);
console.log(`Created task, id is ${task['taskId']}`);

let result = await checkTask(task.taskId, CLIENT_ID, CLIENT_SECRET);
console.log(`Final result: ${JSON.stringify(result)}`);

await downloadResult(result.resultDocumentId, "../../output/node_input_flatten.pdf", CLIENT_ID, CLIENT_SECRET);
console.log("Done and saved to: ../../output/node_input_flatten.pdf");