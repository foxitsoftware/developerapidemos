...
def excelToPDF(doc, id, secret):
	...
	request = requests.post(f"{HOST}/pdf-services/api/documents/create/pdf-from-excel", json=body, headers=headers)
	return request.json()
...
doc = uploadDoc("../../inputfiles/apples.xlsx", CLIENT_ID, CLIENT_SECRET)
print(f"Uploaded doc to Foxit, id is {doc['documentId']}")

task = excelToPDF(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/fromexcel.pdf", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/fromexcel.pdf")