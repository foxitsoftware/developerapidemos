...
def pdfToWord(doc, id, secret):
	...
	request = requests.post(f"{HOST}/pdf-services/api/documents/convert/pdf-to-word", json=body, headers=headers)
	return request.json()
...
task = pdfToWord(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/output.docx", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/output.docx")