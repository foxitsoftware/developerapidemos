...
def pdfToText(doc, id, secret):
	...
	request = requests.post(f"{HOST}/pdf-services/api/documents/convert/pdf-to-text", json=body, headers=headers)
	return request.json()
...
task = pdfToText(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/output.txt", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/output.txt")