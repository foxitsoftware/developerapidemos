...
def flattenPDF(doc, id, secret):
	...
	request = requests.post(f"{HOST}/pdf-services/api/documents/modify/pdf-flatten", json=body, headers=headers)
	return request.json()
...
task = flattenPDF(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/input_flatten.pdf", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/input_flatten.pdf")