...
doc = uploadDoc("../../inputfiles/input.txt", CLIENT_ID, CLIENT_SECRET)
print(f"Uploaded doc to Foxit, id is {doc['documentId']}")

task = textToPDF(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/fromtext.pdf", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/fromtext.pdf")