...
def pptToPDF(doc, id, secret):
	...
	request = requests.post(f"{HOST}/pdf-services/api/documents/create/pdf-from-ppt", json=body, headers=headers)
	return request.json()
...
doc = uploadDoc("../../inputfiles/basic.pptx", CLIENT_ID, CLIENT_SECRET)
print(f"Uploaded doc to Foxit, id is {doc['documentId']}")

task = pptToPDF(doc["documentId"], CLIENT_ID, CLIENT_SECRET)
print(f"Created task, id is {task['taskId']}")
result = checkTask(task["taskId"], CLIENT_ID, CLIENT_SECRET)
print(f"Final result: {result}")

downloadResult(result["resultDocumentId"], "../../output/fromppt.pdf", CLIENT_ID, CLIENT_SECRET)
print("Done and saved to: ../../output/fromppt.pdf")